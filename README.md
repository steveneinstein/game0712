...
When `ADMIN_KEY` is set, open `/login`, choose admin entry, and enter the same key before using Start betting, Roll now, Next round, or Reset game. The key is stored only in that browser tab session and is sent as the `x-admin-key` request header from the admin page.
